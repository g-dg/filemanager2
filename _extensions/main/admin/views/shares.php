	...
	<fieldset><legend>Create Share</legend>
		<form action="'.Router::getHtmlReadyUri('/admin/action/shares').'" method="post">
			...
			<div class="form-inputs">
					...
					<input id="create_path" name="path" type="text" value="" class="u-full-width" required="required" />
...
';
	foreach ($shares as $share_id) {
		...
		echo '<div>';
		echo '<input id="delete_'.htmlspecialchars($share_id).'" name="delete" type="submit" value="Delete" onclick="return confirm(\'Delete share &quot;\'+$(\'#name_'.htmlspecialchars($share_id).'\').attr(\'value\')+\'&quot;?\');" formnovalidate="formnovalidate" style="color: #c00;" />';
		echo '</div>';
